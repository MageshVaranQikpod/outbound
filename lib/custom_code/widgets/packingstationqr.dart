...
class Packingstationqr extends StatefulWidget {
  const Packingstationqr({
    super.key,
    this.width,
    this.height,
    this.orderid,
  ...
  @override
  State<Packingstationqr> createState() => _PackingstationqrState();
}

class _PackingstationqrState extends State<Packingstationqr> {
  ...
  Future<void> _handleScannedValue(String scanValue) async {
    ...
    if (scanValue == widget.packingstation) {
      await _playSound1();
      ...
      // await _showWrongStoreDialog(context, scanValue);
    }
  ...
  Future<void> _showWrongStoreDialog(
    ...
    await showDialog(
      ...
      builder: (alertDialogContext) {
        return AlertDialog(
          title: Text('Wrong Packing Station'),
          content: Text('Scanned Station: $scannedValue'),
          actions: [
            TextButton(
